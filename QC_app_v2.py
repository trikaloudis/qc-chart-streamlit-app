...
def create_qc_chart(data, index_col, date_col, param_col, cl, ucl, lcl, std_dev, violations, applied_rules):
    """
    ...
    fig.add_trace(go.Scatter(
        x=data[index_col], y=data[param_col], mode='lines+markers', name=param_col,
    ...
    for rule in applied_rules:
        ...
        if points:
            ...
            fig.add_trace(go.Scatter(
                x=violation_data[index_col], y=violation_data[param_col],
    ...
    fig.update_layout(
        ...
        xaxis_title=dict(text='Index (Date)', font=dict(size=18)),
    ...
    # Create custom tick labels: "Index (Date)"
    tick_labels = data[index_col].astype(str) + ' (' + data[date_col].dt.strftime('%Y-%m-%d') + ')'
    ...
    fig.update_xaxes(
        ...
        tickangle=-90,
        tickmode='array',
        tickvals=data[index_col], # Positions of the ticks
        ticktext=tick_labels      # The text for the ticks
    )
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        # Corrected raw URL for the logo from the GitHub repository
        logo_url = "https://raw.githubusercontent.com/trikaloudis/qc-chart-streamlit-app/main/Aquomixlab%20Logo%20v2%20white%20font.jpg"
    ...
    if qc_data is not None and historical_limits is not None and spec_limits is not None:
        try:
            # --- Updated Data Validation and Column Identification ---
            if 'Index' not in qc_data.columns or 'Date' not in qc_data.columns:
                st.error("Error: The 'QC data' sheet must contain 'Index' and 'Date' as the first two columns.")
                st.stop()
            
            index_column = 'Index'
            date_column = 'Date'
            qc_data[date_column] = pd.to_datetime(qc_data[date_column])
            parameters = qc_data.columns[2:].tolist() # Parameters start from the 3rd column
            ...
            if selected_parameter:
                ...
                if pd.isna(std_dev) or std_dev == 0:
                    ...
                    fig = create_qc_chart(
                        qc_data, index_column, date_column, selected_parameter,
        ...
        st.markdown("""
        Your data source (Excel or Google Sheet) must contain three sheets with these exact names and structures:

        **1. Sheet: `QC data`**
        - The first column must be named `Index`.
        - The second column must be named `Date`.
        - Subsequent columns should contain the measurement values for each parameter.
        | Index | Date       | Parameter A | Parameter B |
        |-------|------------|-------------|-------------|
        | 1     | 2023-01-01 | 10.2        | 50.5        |
        | 2     | 2023-01-02 | 9.8         | 51.2        |
        | ...   | ...        | ...         | ...         |

        **2. Sheet: `Historical limits`**
        - The first row contains the parameter names.
        - The second row contains the historical mean for each parameter.
        - The third row contains the historical standard deviation for each parameter.
        | Parameter A | Parameter B |
        |-------------|-------------|
        | 10.0        | 50.0        |
        | 0.5         | 1.0         |
        
        **3. Sheet: `Specification limits`**
        - Same structure as `Historical limits`, but with specified mean and standard deviation.
        | Parameter A | Parameter B |
        |-------------|-------------|
        | 10.0        | 50.0        |
        | 1.0         | 2.0         |
